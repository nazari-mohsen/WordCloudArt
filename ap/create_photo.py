...
from photography.models import photo_Mask, photo_main, thumbnail
...
def create_photo(id, content, word1, word2, color1, color2, main_color, bg_color, user):
    ...
    backcolor = bg_color
    name = name_generator(6, time.strftime("%s")) + '.jpeg'
    ...
    if word1 != None:
        sword1 = word1.split(" ")
        w1 = get_display(arabic_reshaper.reshape(sword1[0]))
        words.add(w1, np.random.randint(1, ln_word))
